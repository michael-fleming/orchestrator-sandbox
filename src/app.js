...
const customers = [
  { id: "acme", name: "Acme Corp", email: "orders@acme.example", createdAt: "2026-08-01T08:00:00Z" },
  { id: "globex", name: "Globex Inc", email: "orders@globex.example", createdAt: "2026-08-15T08:00:00Z" },
];
...
/**
 * Filters the customers store using the supported query parameters:
 * - id: exact match on customer id
 * - name: case-insensitive substring match on customer name
...
function queryCustomers(params) {
  ...
  if (q) {
    ...
    result = result.filter((c) =>
      [c.id, c.name, c.email].some((field) => String(field).toLowerCase().includes(needle))
    );
  }

  return _.orderBy(result, [(c) => c.name.toLowerCase()], ["asc"]);
}
